...
public class GameManager : MonoBehaviour
{
    ...
    private SpriteRenderer[] trailSpriteRenderer;
    ...
    void Start()
    {
        Invoke("MusicStart", 2);    //2초뒤에 노래나오게
        ...
        trailSpriteRenderer = new SpriteRenderer[trails.Length];
        for(int i=0; i < trails.Length; i++)
        {
            trailSpriteRenderer[i] = trails[i].GetComponent<SpriteRenderer>();
        }
    }
    ...
    void Update()
    {
        ...
        for(int i=0; i< trailSpriteRenderer.Length; i++)
        {
            Color color = trailSpriteRenderer[i].color;
            color.a -= 0.01f;
            trailSpriteRenderer[i].color = color;
        }
    }

    public void ShineTrail(int index)
    {
        Color color = trailSpriteRenderer[index].color;
        color.a = 0.32f;
        trailSpriteRenderer[index].color = color;
    }
